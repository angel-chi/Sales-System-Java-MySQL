...
Descripciòn: Al entrar al espacio de generar una nueva venta, esta permitìa llenar los campos pero al intentar generarla no realizaba acciòn.
Soluciòn: El problema se generaba debido a que se estaban tomando los campos nùmericos con entradas incorrectas por lo que se quedaba congelado asì que se solucionò un try-catch que permitìa leer los datos correctos o imprimir un error.
...
Soluciòn: Se añadiò in try-catch que intentaba buscar los identificadores dentro de la base de datos y, sin no los encuentra, arroja un error.
...
Soluciòn: Como la acciòn de agregar un producto solo toma las entradas y llama al constructor, lo que se hizo fue, antes de llamar al constructor del objeto, hacer una verificaciòn que los tipos de datos ingresados coincidan con el tipo de dato establecido en la clase, para posteriormente llamar al constructor y ahora sì ingresarlo en la base de datos.
...
La interfza originalpermitìa ingresar un cliente, vendedor o producto nuevo incluso si los campos de informaciòn estaban vacìos. Asì que se decidiò agregar una nueva clase abstracta Validator con un mètodo validate, que solo tiene con entrada una entidad y un mètodo nombrado validate, el cual es heradado en todas las demàs clases con el propòsito de verificar que los campos de informaciòn no se encuentran vacìos. Si los campos no son llenados por completo, antes de ejecutar una acciòn en la base de datos, se abre la ventana emergernte de que la acciòn no se puede concretar.

#### Comprobaciòn de datos
El programa maneja la entrada de datos como entradas String lo que provoca que no se de ningùn error sin importar què dato se ingrese, sin embargo, hay campos que deben de tener un formato especìfico, por ejemplo el telèfono en los vendedores deben ser puros nùmeros y entre 8 y 15 dìgitos, los id deben ser una secuencia de nùmeros y, en el producto el precio o existencias no deben ser negativos ni 0 (por coherencia). Asì que lo que se hizo fue implementar nuevas funciones sobre la validate ya existente, que se asegurara que los datos cumplieran estas caracterìsticas, si en el proceso de validaciòn se encontraba una contradicciòn inmediatamente se termina el proceso y se lanza una ventana emergente al usuario con la advertencia de que la informaciòn es incorrecta.

#### Verificaciòn antes de eliminar
Desde la gestiòn de producto cliente y vendedor existe la acciòn de eliminar a cada uno de estos, desde su vena correpondiente, lo que observamos es que se podìa seleccionar y simplemente eliminar la informaciò, lo que consideramos un tanto peligroso en el uso real, por ello implementamos una funciòn que, dependiendo del resgitro seleccionado para eliminar, ejecuta una ventana emergente que indica que la acciòn que se realiza es la eliminaciòn y el nombre del producto o la persona para que e usuario vea què es lo que se eliinarà, pidiendo la confirmaciòn para realizar la eliminaciò o, de lo contrario simplemnte quedarse en la ventana esperando una nueva acciòn.

#### Espacio de bùsqueda en la generaciòn de ventas
Ademàs de la soluciòn del problema inicial que se tenìa en este espacio se añadiò una funcionalidad para que el usurio pueda buscar el id o el nombre tanto del prducto como del cliente. En caso de que buscara por algo que no se encuentra registrado en la base de datos lo que se hace es abrir una ventana que explic que no està registrado y se le prroporciona la opciòn de agregarlo como un elemento nuevo, si el usuario lo acepta, se redirige tanto a la venta como el proceso de gestiòn de clientes o productos, segùn sea el caso.
