...
public class CustomerDAO implements CRUD<Customer> {
    ...
    public boolean create(Customer entity) {
        ...
        try (Connection conn = DBConnection.connection();
            ...
            if (rows_affected>0){
                ...
                MenuController.setAlert(Alert.AlertType.ERROR, "Error al añadir cliente: ");
                return false;
            }

        }catch (SQLException e){
            MenuController.setAlert(Alert.AlertType.ERROR, "Error al añadir cliente: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean update(Customer entity) {
        ...
        try(Connection conn = DBConnection.connection();
            ...
            if (rows_affected>0){
                MenuController.setAlert(Alert.AlertType.CONFIRMATION, "Ciente actualizado correctamente");
                return true;
            }else{
    ...
    public void setTable(ObservableList<Customer> customers) {
        ...
        try (Connection conn = DBConnection.connection();
            ...
            MenuController.setAlert(Alert.AlertType.ERROR, "Error al configurar la tabla de clientes: " + e.getMessage());
        }
