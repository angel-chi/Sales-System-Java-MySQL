...
public class SellerDAO implements CRUD<Seller> {
    ...
    public boolean create(Seller entity) {
        ...
        try (Connection conn = DBConnection.connection();
            ...
            if (rows_affected>0){
                MenuController.setAlert(Alert.AlertType.CONFIRMATION, "Vendedor agregado de manera correcta");
                return true;
            }else{
    ...
    public boolean update(Seller entity) {
        ...
        try(Connection conn = DBConnection.connection();
            ...
            if (rows_affected>0){
                MenuController.setAlert(Alert.AlertType.CONFIRMATION, "Vendedor actualizado de manera correcta");
                return true;
            }else{
                MenuController.setAlert(Alert.AlertType.ERROR, "Error al actualizar vendedor ");
                return false;
            }

        }catch (SQLException e){
            MenuController.setAlert(Alert.AlertType.ERROR, "Error al actualizar vendedor: " + e.getMessage());
            return false;
        }
    ...
    public boolean delete(String id) {
        ...
        try(Connection conn = DBConnection.connection();
            ...
            if (rows_affected>0){
                MenuController.setAlert(Alert.AlertType.CONFIRMATION, "Vendedor eliminado de manera correcta");
                return true;
            }else{
                MenuController.setAlert(Alert.AlertType.ERROR, "Error al eliminar vendedor ");
                return false;
            }
            ...
            MenuController.setAlert(Alert.AlertType.ERROR, "Error al eliminar vendedor: " + e.getMessage());
            return false;
        }
    ...
    public void setTable(ObservableList<Seller> sellers){
        ...
        try (Connection conn = DBConnection.connection();
            ...
            MenuController.setAlert(Alert.AlertType.ERROR, "Error al crear la tabla Vendedor: " + e.getMessage());
        }
    }
    ...
    public static boolean login(String dni,String user){
        ...
        if (dni ==null || user ==null || dni.isEmpty()||user.isEmpty() ){
            MenuController.setAlert(Alert.AlertType.ERROR,"El ID Vendedor o Contraseña están vacíos");
            return false;
        }
        ...
        try (Connection conn = DBConnection.connection();
            ...
            try (ResultSet rs = pstmt.executeQuery()){
                if (rs.next()){
                    ...
                    MenuController.setAlert(Alert.AlertType.ERROR, "ID Vendedor no encontrado") ;
                    return false;
                }
            }
        }catch (SQLException e){
            MenuController.setAlert(Alert.AlertType.ERROR, "Error al buscar vendedor: " + e.getMessage());
            return false;
        }
